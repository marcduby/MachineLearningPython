...
def get_scores(model, Xtrain, Xtest, ytrain, ytest, scoring):
    ...
    print("\nin {} for model {} got train score {}, test score {} and f1 score {}".format(hf.format_timespan(end-start, detailed=True), model.__class__.__name__, score_train, score_test, score_f1))
...
if __name__ == "__main__":
    ...
    model_list = [('kneighbors', KNeighborsClassifier()), ('svc', SVC(random_state=random_state, gamma='scale'))]
    final_model = LogisticRegression()
    model = StackingClassifier(estimators=model_list, final_estimator=final_model, verbose=True)
    model.fit(X_train, y_train)
