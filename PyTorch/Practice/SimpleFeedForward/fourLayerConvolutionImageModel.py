...
class ThreeHiddenLayerConvolutional(nn.Module):
    def __init__(self):
        ...
        self.conv_02 = nn.Sequential (
            nn.Conv2d(
                in_channels=16,   # the one grayscale channel
                out_channels=32,  # will output 32 channels
                kernel_size=3,   # the size of the moving filter 3x3
                stride=1,        # how much the filter moves by step
                padding=1
            ),                   # output will be (32, 16, 16)
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),     # height and width reduced by factor of 2, output will be (32, 8, 8)
        )
