...
# file input
# file_name = "part-00000-24063efa-89ff-412d-9134-8cd90f09380b-c000.json"
file_name = "part-00000-7d12a038-af43-4b1f-9327-721b3d22f181-c000.json"
file_input = dir_data + "dig-analysis-data/out/varianteffect/common/" + file_name
file_twobit = dir_data + 'Basset/Production/hg19.2bit'
file_output = dir_data + "dig-analysis-data/out/regionpytorch/deepsea_varianteffect/deepsea_" + file_name
# file_model_weights = dir_data + 'DeepSEA/Models/deepsea_cpu.pth'
file_model_weights = dir_data + 'DeepSEA/Models/deepsea_predict.pth'
labels_file = dir_data + 'DeepSEA/Models/deepsea_labels.txt'
...
# configure argparser
parser = argparse.ArgumentParser("script to run the DeepSEA variant effect PyTorch model on DCC a variants file")
# add the arguments
parser.add_argument('-i', '--input_file', help='the file to process', default=file_input, required=False)
parser.add_argument('-o', '--output_file', help='the file to save the results to', default=file_output, required=False)
