...
def create_sentence_from_beta_prob_row(row, log=False):
    ...
    '''
    sentence = ""
    sentence_template = "gene {} has a {} genetic association with {}"
    direction = ""
    ...
    if row.get('gene') and row.get('phenotype_name') and row.get('prob'):
        ...
        sentence = sentence_template.format(row.get('gene'), adjective, row.get('phenotype_name'))
...
def get_list_of_gene_association_sentences(conn, string_select, has_beta = True, log=False):
    ...
    for row in db_results:
        ...
        sentence = create_sentence_from_beta_prob_row(map_row)
        if len(sentence) > 5:
...
def get_list_of_gene_pathway_sentences(conn, string_select, log=False):
    ...
    for row in db_results:
        ...
        sentence = create_sentence_from_gene_pathway_row(map_row)
        if len(sentence) > 5:
            list_sentences.append(sentence)
...
if __name__ == "__main__":
    ...
    # get genebass data 
    list_temp = get_list_of_gene_association_sentences(conn, SQL_SELECT_GENEBASS, log=False)
    print("to process, got genebass list of size: {}".format(len(list_temp)))
    list_sentences = list_sentences + list_temp

    # list_temp = get_list_of_gene_association_sentences(conn, SQL_SELECT_600k, log=False)
    # print("to process, got 600k list of size: {}".format(len(list_temp)))
    # list_sentences = list_sentences + list_temp

    # list_temp = get_list_of_gene_association_sentences(conn, SQL_SELECT_MAGMA, has_beta=False, log=True)
    # print("to process, got magma list of size: {}".format(len(list_temp)))
    # list_sentences = list_sentences + list_temp

    list_temp = get_list_of_gene_pathway_sentences(conn, SQL_SELECT_PATHWAY_GENES, log=True)
    print("to process, got pathway gene list of size: {}".format(len(list_temp)))
    list_sentences = list_sentences + list_temp
    ...
    # create_json_dataset_file(list_sentences, FILE_DATA)
