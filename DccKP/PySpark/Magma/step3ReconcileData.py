...
df_ontology = df_ontology.filter(df_ontology.PH == phenotype)
df_ontology.show()

# join the ontology with the gene values
df_phenotype_pvalue = df_pvalue.join(df_ontology, how='outer')
...
df_export.show()
df_export = df_export.select('gene', 'geneNcbiId', 'nParam', 'subjects', 'zStat', 'pValue', 'phenotypeEfoId', 'phenotypeMondoId') \
