...
def main():
    ...
    for ancestry in ancestries:
        if ancestry in map_ancestry.keys():
            ...
            file_output = f'{dir_cojo}/output/out_{phenotype}_{ancestry}'
            ...
            cojo_command = f'{dir_cojo}/gcta_1.93.2beta/gcta64 --bfile {file_g1000} --cojo-file {file_input} --cojo-wind 500 --threads 8 --cojo-slct --out {file_output}'
            run_system_command(cojo_command, if_test = arg_if_test)

            # copy results to s3 new directory
            s3_upload_command = f'aws s3 cp --recursive --exclude "*" --include "out_{phenotype}_{ancestry}*.*" {dir_cojo}/output/ s3://{dir_s3_outputs}/ancestry={ancestry}/'
